  ...
  var shader:FlxSprite;
	...
	override public function create()
	{
      ...
      case 'normal':
            ...
            var groundd = new FlxSprite(-500, 50).loadGraphic(Paths.image('stage/ak'));
            groundd.scale.set(1, 0.4);
            add(groundd);
            
            var ss = new FlxSprite(-200, -120).loadGraphic(Paths.image('stage/oh'));
            ss.scale.set(0.6, 0.6);
            add(ss);
            
            var soo = new FlxSprite(-20, -100).loadGraphic(Paths.image('stage/sofa'));
            soo.scale.set(0.6, 0.6);
            add(soo);
            
            var luumi = new FlxSprite(-160, 120);
            luumi.frames = Paths.getSparrowAtlas('stage/luumis');
            luumi.animation.addByPrefix('anim', 'idle', 24, true);
            luumi.animation.play('anim');
            add(luumi);
            
            var pudim = new FlxSprite(920, 257.5).loadGraphic(Paths.image('stage/pudim'));
